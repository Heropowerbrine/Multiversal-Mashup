...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "runHaxeCode", function(codeToRun:String) {
			...
			try {
				var myFunction:Dynamic = HScript.expr(codeToRun);
				myFunction();
			}
			catch (e:Dynamic) {
				switch(e)
		...
		Lua_helper.add_callback(lua, "noteTweenZ", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];

			if(testicle != null) {
				PlayState.instance.modchartTweens.set(tag, FlxTween.tween(testicle, {z: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
						PlayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "reloadhitboxes", function() {
			PlayState.instance.reloadhitboxes();
	        });
...
class HScript
{
	...
	public static function expr(coderun):Dynamic
	{
		@:privateAccess
