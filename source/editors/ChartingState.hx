...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		UI_box.x = (gridBG.x + (40 + 40 * 9 * 2)) + 40 / 2;
		UI_box.y = 25;
		UI_box.scrollFactor.set();

		bpmTxt.x = UI_box.x + UI_box.width + 40;
	...
	function addSongUI():Void
	{
		...
		var stepperMania:FlxUINumericStepper = new FlxUINumericStepper(100, 70, 1, 0, 0, 9, 1);
	...
	function addSectionUI():Void
	{
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			{
				var boob = note[1];
				if (boob>Note.keyAmmo[_song.mania]-1){
					boob -= Note.keyAmmo[_song.mania];
				}else{
					boob += Note.keyAmmo[_song.mania];
				}
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				var boob = note[1]%Note.keyAmmo[_song.mania];
				boob = Note.keyAmmo[_song.mania]-1 - boob;
				if (note[1] > Note.keyAmmo[_song.mania]-1) boob += Note.keyAmmo[_song.mania];
	...
	function addChartingUI() {
		...
		var vocalsExtraOne = new FlxUICheckBox(10 + 120, 90 + 20, null, null, "Extra Voices 1", 100);
		vocalsExtraOne.checked = true;
		vocalsExtraOne.callback = function()
		{
			//vocalsExtraOne.checked = !vocalsExtraOne.checked;
			if (extraVocals[0] != null)
			{
				var vol:Float = 1;

				if (!vocalsExtraOne.checked)
					vol = 0;

				extraVocals[0].volume = vol;
			}
		};

		var vocalsExtraTwo = new FlxUICheckBox(10 + 120, 90 + 40, null, null, "Extra Voices 2", 100);
		vocalsExtraTwo.checked = true;
		vocalsExtraTwo.callback = function()
		{
			//vocalsExtraTwo.checked = !vocalsExtraTwo.checked;
			if (extraVocals[1] != null)
				{
					var vol:Float = 1;
	
					if (!vocalsExtraTwo.checked)
						vol = 0;
	
					extraVocals[1].volume = vol;
				}
		};

		var vocalsExtraThree = new FlxUICheckBox(10 + 120, 90 + 60, null, null, "Extra Voices 3", 100);
		vocalsExtraThree.checked = true;
		vocalsExtraThree.callback = function()
		{
			//vocalsExtraThree.checked = !vocalsExtraThree.checked;
			if (extraVocals[2] != null)
				{
					var vol:Float = 1;
	
					if (!vocalsExtraThree.checked)
						vol = 0;
	
					extraVocals[2].volume = vol;
				}
		};

		var vocalsExtraFour = new FlxUICheckBox(10 + 120, 90 + 80, null, null, "Extra Voices 4", 100);
		vocalsExtraFour.checked = true;
		vocalsExtraFour.callback = function()
		{
			//vocalsExtraFour.checked = !vocalsExtraFour.checked;
			if (extraVocals[3] != null)
				{
					var vol:Float = 1;
	
					if (!vocalsExtraFour.checked)
						vol = 0;
	
					extraVocals[3].volume = vol;
				}
		};

		var vocalsExtraFive = new FlxUICheckBox(10 + 120, 90 + 100, null, null, "Extra Voices 5", 100);
		vocalsExtraFive.checked = true;
		vocalsExtraFive.callback = function()
		{
			//vocalsExtraFive.checked = !vocalsExtraFive.checked;
			if (extraVocals[4] != null)
				{
					var vol:Float = 1;
	
					if (!vocalsExtraFive.checked)
						vol = 0;
	
					extraVocals[4].volume = vol;
				}
		};
		...
		tab_group_chart.add(vocalsExtraOne);
		tab_group_chart.add(vocalsExtraTwo);
		tab_group_chart.add(vocalsExtraThree);
		tab_group_chart.add(vocalsExtraFour);
		tab_group_chart.add(vocalsExtraFive);
	...
	function loadSong():Void
	{
		...
		if (_song.song.toLowerCase() == 'godspeed')
			vocalsToAdd = ['-super', '-blast', '-kaio', '-eater', '-tricky'];
		else if (_song.song.toLowerCase() == "what i wanna know is where's the caveman")
			vocalsToAdd = ['-new', '-sooth', '-erection', '-where'];
		else 
			vocalsToAdd = [];
		...
		{
			var f:Dynamic = Paths.voices(currentSongName, i);
			var v = new FlxSound();
			if (Std.isOfType(f, Sound) || OpenFlAssets.exists(f)) {
				v.loadEmbedded(f);
	...
	override function update(elapsed:Float)
	{
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(curSelectedNote != null) {
				...
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Note.keyAmmo[_song.mania];
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < Note.keyAmmo[_song.mania]? -0.3 : 0.3; //would be coolio
	...
	*/
	...
	function reloadGridLayer() {
		gridLayer.clear();
		//gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Note.keyAmmo[_song.mania] * 2, Std.int(GRID_SIZE * 32 * zoomList[curZoom]));
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Note.keyAmmo[_song.mania] * 2, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Note.keyAmmo[_song.mania] * 2, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE * GRID_SIZE + GRID_SIZE * Note.keyAmmo[_song.mania] * 2), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
			gridLayer.add(gridBlack);
		}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * Note.keyAmmo[_song.mania])).makeGraphic(2, leHeight, FlxColor.BLACK);
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo % Note.keyAmmo[_song.mania], null, null, true);
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Note.keyAmmo[_song.mania];
			for (i in _song.notes[curSec].sectionNotes)
			{
